...
private:
    ...
    geometry_msgs::PoseStamped current_pose2_;
...
public:
    OffboardController() : nh_(""), current_pose_(), current_state_(),current_pose2_() {
        state_sub_ = nh_.subscribe<mavros_msgs::State>("uav2/mavros/state", 10, &OffboardController::stateCallback, this);
        local_pos_pub_ = nh_.advertise<geometry_msgs::PoseStamped>("uav2/mavros/setpoint_position/local", 10);
        ...
        pose2_sub_ = nh_.subscribe<geometry_msgs::PoseStamped>("uav2/mavros/local_position/pose", 10, &OffboardController::pose2Callback, this);

        path_pub_ = nh_.advertise<nav_msgs::Path>("uav2/path", 10); //path
        point_pub_ = nh_.advertise<geometry_msgs::PointStamped>("uav2/point", 10); //point
    }
    ...
    void run() {
        ...
        while (ros::ok()) {
            ...
            path.poses.emplace_back(current_pose2_);
